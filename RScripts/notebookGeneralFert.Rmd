...
```
The vertical lines on the plots, corresponding to horizontal lines on the map are features that I'm highlighting. The grey lines in the 3rd figure are what the average fraction of each fertilization type is.
Firstly we see a broad peak around 2.5 which represents the north south chunk of the British isles containing Ireland. In the second plot, we see that north of the green line (the Shetlands) becomes mostly mixed and self. But it should be noted this only corresponds to ~10 species. We see a second peak around 7.8, which corresponds to species generally found only at the north coast of mainland Scotland. This could be due to this area now including species mostly exclusive to the Orkney islands. 
...
cat("EW Index > 5? (rows normalised)")
prop.table(EWgreaterThan5VFert,1)
cat("\n\n\nChisq EW>5 vs FertMode")
chisq.test(df$myFertGen, df$EWGreaterThan5)

cat("\n\n\nStd. dev. of EW Index by fert mode:\n self, mixed, cross\n")
...
```
Bizarrely, the EW index only ranges across Britain, not including Ireland (this is as far as I can tell). It goes from East to West so 0 is Yarmouth and 10 is Penzance. There is a statistically significant fraction more selfers of index greater than 5, shown as west of the red line. This red line cuts off east England which has signifantly less rainfall. We can investigate the effect of precipitation now.
```{r}
df %>% filter(!is.na(myFertGen)) %>% filter(!is.na(Annual.precipitation)) %>% ggplot(aes(x=myFertGen, y=Annual.precipitation)) + geom_boxplot()
df %>% filter(!is.na(myFertGen)) %>% filter(!is.na(Annual.precipitation)) %>% ggplot() + geom_density(aes(x=Annual.precipitation, fill=myFertGen), alpha=0.4)
df %>% filter(!is.na(myFertGen)) %>% filter(!is.na(Annual.precipitation)) %>% ggplot(aes(Annual.precipitation, after_stat(count), fill = myFertGen)) +
  geom_density(position = "fill") + geom_hline(yintercept = c(crossFrac, 1-selfFrac), linetype="dashed", color="grey", size=0.5)



df %>% filter(!is.na(myFertGen)) %>% filter(!is.na(myPlantAtPrecip)) %>% ggplot(aes(x=myFertGen, y=myPlantAtPrecip)) + geom_boxplot()
df %>% filter(!is.na(myFertGen)) %>% filter(!is.na(myPlantAtPrecip)) %>% ggplot() + geom_density(aes(x=myPlantAtPrecip, fill=myFertGen), alpha=0.4)
df %>% filter(!is.na(myFertGen)) %>% filter(!is.na(myPlantAtPrecip)) %>% ggplot(aes(myPlantAtPrecip, after_stat(count), fill = myFertGen)) + geom_density(position = "fill") + geom_hline(yintercept = c(crossFrac, 1-selfFrac), linetype="dashed", color="grey", size=0.5)
```
